...
function logDeletion($username, $path, $success) {

    $username = $_SESSION["user"];
    global $connection; // use the global connection variable
    $relativePath = getRelativePath($path); // Get the relative path
    $stmt = $connection->prepare("INSERT INTO delete_log (username,filename, delete_time, result) VALUES (:username, :pathe, DATE_ADD(NOW(), INTERVAL 7 HOUR), :success)");
    $stmt->bindParam(':username', $username);
    $stmt->bindParam(':pathe', $relativePath);
...
function getRelativePath($fullPath) {
    $startFrom = 'HMS';
    $position = strpos($fullPath, $startFrom);
    if ($position !== false) {
        return substr($fullPath, $position);
    } else {
        return $fullPath; // or handle this case as needed
    }
}
...
class config
{
	...
	self::$root = real_path(self::$config['root']);
        ...
        if ($is_doc) {
            self::$assets = empty(self::$config['assets']) ? '/js/' : rtrim(self::$config['assets'], '/') . '/';
        }
...
function safe_iptc_tag($val)
{
    ...
    return @mb_detect_encoding($val, 'UTF-8', true) ? $val : @mb_convert_encoding($val, 'UTF-8');
}
...
if (post('action')) {
    ...
    if ($action === 'fm') {
        ...
        if ($task === 'upload') {
            ...
            if ($is_dir) {
                ...
                // recursive rmdir
                function rrmdir($dir, &$success, &$fail, $username)
                {
                    ...
                    if (!empty($files)) {
                        foreach ($files as $file) {
                            if (is_dir("$dir/$file")) {
                                rrmdir("$dir/$file", $success, $fail, $username);
                            } else {
                                $result = @unlink("$dir/$file");
                                $result ? $success++ : $fail++;
                                logDeletion($username, "$dir/$file", $result);
                            }
                        }
                    }
                    $result = @rmdir($dir);
                    $result ? $success++ : $fail++;
                    logDeletion($username, $dir, $result);
                }
        
                // recursive rmdir start
                rrmdir($path, $success, $fail, $username);
            ...
            // single file
                ...
                $result = @unlink($path);
                fm_json_toggle($result, 'PHP unlink() failed');
                logDeletion($username, $path, $result);
            }
    ...
    */
        ...
        <body class="body-loading">
            ...
            <?php
            // load _files/js/custom.js if it exists
            get_include('js/custom.js');

            // Load all Javascript assets
            $javascriptAssets = array_filter([
                'toastify.min.js',
                'sweetalert2.min.js',
                'anime.min.js',
                'list.min.js',
                ...
                'localizedFormat.js',
                'relativeTime.js',
                (in_array(config::$config['download_dir'], ['zip', 'files']) ? 'js-file-downloader.min.js' : false),
                (config::$config['download_dir'] === 'browser' ? 'jszip.min.js' : false),
                (config::$config['download_dir'] === 'browser' ? 'FileSaver.min.js' : false),
                'meta.js',
            ]);

            // Output the script tag for each asset
            foreach ($javascriptAssets as $javascriptAsset) {
                // Output the script tag for all other assets
                echo '<script src="' . config::$assets . $javascriptAsset . '"></script>';
            }

            // Output the script tag for your local 'files.js'
            echo '<script src="js/files.js"></script>';
            ?>
